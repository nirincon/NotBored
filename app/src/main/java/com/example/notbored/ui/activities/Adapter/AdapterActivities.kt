...
class AdapterActivities (val activitiesList: List<String>, var onActivityListener : OnActivityListener ): RecyclerView.Adapter<ActivitiesViewHolder> () {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ActivitiesViewHolder {
        ...
        val itemBinding = TemplateActivityItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        val holder = ActivitiesViewHolder(itemBinding)

        itemBinding.activity.setOnClickListener {
            val position = holder.adapterPosition.takeIf { it != DiffUtil.DiffResult.NO_POSITION }
                ?: return@setOnClickListener
            onActivityListener.onActivityListener(activitiesList[position])
        }
        return holder
    }
